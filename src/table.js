...
module.exports = function(data = {}, columns = { '-': 76 }, opt = {}){
  ...
  dataKeys.forEach((key, i, arr) => {
    if (opt.repeat && i % opt.repeat === 0 && i != 0) {
      header(columns, opt, true);
    }
    let rowArr = [key]
    let row = data[key]
    for (entry in row) rowArr.push(row[entry])
    log(rowArr.reduce((all, ent, i) => {
      if (ent instanceof Array) ent = ent.join(' + ');
      all = all + p.v + ' ' + fixed(ent, widths[i]) + ' ';
      return all;
    }, '') + p.v);
  });
